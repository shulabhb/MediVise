...
def should_use_memory(message: str) -> bool:
    """Lightweight heuristic to decide whether to use memory/docs for a message."""
    try:
...
Please return only valid JSON without any additional text."""
    ...
    async def summarize_medical_document(self, document_text: str) -> Dict[str, Any]:
        """
        Fast path: If document fits in context, single JSON call. Otherwise map-reduce.
        Always renders deterministically to avoid duplicate headings.
        """
        try:
            logger.info(f"Summarizing document with {len(document_text)} characters")
            
            # Fast path: docs under 8000 chars → single JSON call (much faster)
            if len(document_text) < 8000:
                logger.info("Using fast single-pass summarization")
                summary_json = await self._summarize_single_pass(document_text)
            else:
                logger.info("Using map-reduce for large document")
            ...
            # Render to markdown deterministically
    ...
    async def _summarize_single_pass(self, document_text: str) -> SummaryResponse:
        """Fast single-pass JSON summarization for docs that fit in context."""
        try:
            # De-identify PHI
            deidentified_text, redactions_applied = deidentify_phi(document_text)
            
            # Simple prompt for single pass
            user_prompt = f"""Summarize this medical document in patient-friendly language.

DOCUMENT:
{deidentified_text}

Return JSON with sections: Summary, Findings, What it means, Key Medications, Important Instructions or Precautions, Warnings or Side Effects, Red flags, Next steps, Key highlights.

Each section should have a "title" and "bullets" array. Only include sections with content."""
            
            result = await self._run_json_prompt(SUMMARY_SYSTEM, user_prompt, max_retries=2)
            ...
            logger.error(f"Error in single-pass summarization: {e}")
            raise
